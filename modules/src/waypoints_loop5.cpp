 ...
 *	Date:		17-10-2014
 *	Version:	5.0
 *		The one with parabolic interpolation path planning
 */

#include "waypoints_loop5.h"
...
namespace waypoints_loop5_globals {
...
using namespace waypoints_loop5_globals;

/* Add configure function*/
namespace waypoints_loop5_functions {
	void loadParameters(std::string fileName) {
		...
		ConfigParser::loadParameters("WAYPOINTS_LOOP5", &parameters, fileName);
	}
}
using namespace waypoints_loop5_functions;
 ...
 *		beginning.
 */
void waypoints_loop5(hardware &hardware_list, Logger &log, deque<coord> &waypoints_list, string config_filename) {	
	...
	if (waypoints_list.size() > 1) {
		path = get_path(currentCoord, waypoints_list[wp_it], waypoints_list[wp_it +1]);
	} else if(!waypoints_list.empty()) {
		path = get_path(currentCoord, waypoints_list[wp_it], currentCoord);
	}
		...
		while(!exitProgram) {
			...
			switch(state) {
				...
				case 1:
					...
					flightVector = get_velocity(&controller, currentCoord, path, 10, 10, SPEED_LIMIT_);
