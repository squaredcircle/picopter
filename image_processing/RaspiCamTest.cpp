...
CAMERA_STREAM::CAMERA_STREAM() {
	...
	this->MIN_VAL		= 143;
	this->MAX_VAL		= 255;
	this->PIXLE_THRESHOLD	= 10;
	
    this->PROCESS_IMAGE_REDUCE	= 0.5;
    this->STREAM_IMAGE_REDUCE	= 1;
...
void CAMERA_STREAM::setDisplay(bool display) {
	boost::mutex::scoped_lock lock(process_mutex);
	this->display = display;
}

void CAMERA_STREAM::setStream(bool stream) {
	boost::mutex::scoped_lock lock(process_mutex);
	this->stream = stream;
}
...
double CAMERA_STREAM::getFramerate() {
	boost::mutex::scoped_lock lock(process_mutex);
	std::cout << "No frames:\t" << frame_counter << std::endl;
	std::cout << "Time:\t" << difftime(end_time, start_time) << std::endl;
	std::cout << "Framerate:\t" << frame_counter/difftime(end_time, start_time) << std::endl;
...
void CAMERA_STREAM::processImages() {	
	...
	while(running && frame_counter<10000+1) {
		process_mutex.lock();
...
int main(int argc, char* argv[]) {
	...
	double numFrames;
	while(!exitProgram) {
		numFrames = cam.getFramerate();
		for (int i=0; i<10 && !exitProgram; i++) {
			boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
		}
		if(numFrames > 10*1000) break;
	}
	cam.stop();
	cam.close();
	return 0;
