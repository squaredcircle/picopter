 ...
 * 
 * Changes since v1.1:
 * We now read parameters (max hue, pixle threshold) from a text file.
 * 
 * TODO:
...
#define PARAMETER_FILE "camera_threshold_list.txt"
...
void readParameters(string fileName, int *MIN_HUE, int *MAX_HUE, int *MIN_SAT, int *MAX_SAT, int *MIN_VAL, int *MAX_VAL, int *PIXLE_THRESHOLD) {
	ifstream waypointsFile(fileName.c_str());
	istringstream iss;
	string line;
	char comment = '#';
	...
	for(int i=0; i<6; i+=2) {
		...
		while(line.at(0) == comment) { waypointsFile >> ws; getline(waypointsFile, line);}
		iss.clear();
		iss.str(line);
...
bool getRedCentre(Mat& Isrc, const uchar table_reduce_colorspace[],  const uchar table_threshold[][LOOKUP_SIZE][LOOKUP_SIZE], int *centre_row, int *centre_col, int PIXLE_THRESHOLD) {
	int nRows = Isrc.rows;
	int nCols = Isrc.cols;
...
int main(int argc, char** argv) {
	...
	readParameters(PARAMETER_FILE, &MIN_HUE, &MAX_HUE, &MIN_SAT, &MAX_SAT, &MIN_VAL, &MAX_VAL, &PIXLE_THRESHOLD);
	...
	while(true) {
		...
		if(getRedCentre(image, lookup_reduce_colourspace, lookup_hsv, &cross_row, &cross_col, PIXLE_THRESHOLD)) {
